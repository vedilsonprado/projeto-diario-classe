...
public class AlunoService {
	...
	/**
	 * Lista os Alunos pertencentes a uma Turma específica (chave para a exibição no
	 * frontend).
	 */
	...
	public Aluno buscarPorId(Long id) {
		return alunoRepository.findById(id).orElseThrow(
				() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Aluno não encontrado para o ID: " + id));
	}
	...
	public Aluno atualizar(Long id, Aluno alunoDetalhes) {
		...
		// Atualiza a Turma, se fornecida (precisa de validação adicional de Turma,
		...
		return alunos.stream().map(aluno -> {
			...
			for (TurmaUC tuc : turmasUcs) {
				...
				long countCC = aluno.getLancamentosNotas().stream()
						// CAMINHO NOVO: criterio -> capacidade -> unidadeCurricular
						.filter(n -> n.getCriterio().getCapacidade().getUnidadeCurricular().getId().equals(uc.getId()))
						.filter(n -> n.getCriterio().getTipoAvaliacao() == TipoAvaliacao.CRITICO)
						.filter(n -> Boolean.TRUE.equals(n.getAtingiu())).count();
